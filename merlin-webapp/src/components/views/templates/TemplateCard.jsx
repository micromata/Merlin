...
class TemplateCard extends React.Component {
    ...
    buildItem = (label, content) => {
        return <li className="list-group-item">{label}{content.map((line, index) => {
            return <div className="card-list-entry" key={index}>{line[0]}<br/><span className="card-list-entry-value">{line[1]}</span>
            </div>;
        })}</li>;
    }
    ...
    render = () => {
        ...
        let content = null;
        if (!templateId) {
            templateId = template.fileDescriptor.filename;
            content = [['Filename', template.fileDescriptor.filename]];
        } else {
            content = [['Id', template.id]];
            content.push(['Filename', template.fileDescriptor.filename]);
        }
